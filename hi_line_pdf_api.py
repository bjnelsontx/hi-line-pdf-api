...
def generate_statements():
    input_zip = request.files.get('input_zip', None)
    zip_url = request.json.get('zip_url') if request.is_json else None

    if not input_zip and not zip_url:
        return "Missing file or URL: Please upload a ZIP or provide a Google Drive URL.", 400
    ...
    with TemporaryDirectory() as tmpdir:
        ...
        if zip_url:
            try:
                ...
                return f"Failed to fetch ZIP from URL: {str(e)}", 400
        else:
        ...
        if not excel_file or not logo_file:
            return "ZIP must include one Excel (.xlsx) file and one Logo (.jpg/.jpeg/.png).", 400
